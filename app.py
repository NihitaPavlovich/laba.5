...
app = Flask(__name__)
conn = psycopg2.connect(database="laba_4",
                        user="postgres",
                        password="666",
                        host="localhost",
                        port="5432")
...
def login_post():
    ...
    if records == []:
        return render_template('account.html', full_name=None, login=None, password=None,
...
def register_post():
    ...
    conn.commit()
    return render_template('account.html', full_name=full_name, login=username, password=password, error=None)
